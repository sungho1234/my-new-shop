...
// ALL_PRODUCTS (로컬 데이터 – 실제 제품 ID/상세 맞춰서 확장하세요)
const ALL_PRODUCTS = [
  {
    id: 'general-growth',
    title: '일반인의 성장책',
    author: '박명수',
    price: '29,900원',
    thumbnail: '/images/general-growth.jpg', // 실제 이미지 경로
  },
  // 다른 제품 추가 예시 (DB productId에 맞게)
  // {
  //   id: 'crypto-master',
  //   title: '크립토 마스터',
  //   author: '김철수',
  //   price: '49,900원',
  //   thumbnail: '/images/crypto-master.jpg',
  // },
];
...
const mergeWishlistWithProducts = (wishlist: any[]) => {
  ...
  const merged = wishlist.map(item => {
    ...
    console.warn("⚠️ 매치 실패: ", item.productId, " – ALL_PRODUCTS에 추가하세요");
    return { ...item, title: item.title || 'Unknown Product', author: 'Unknown', price: '0원', thumbnail: '/placeholder.png' };
  }).filter(Boolean);
  console.log("✅ 병합 완료: 길이 =", merged.length, ", merged[0] =", merged[0] || '빈 배열');
  return merged;
};
...
const MyPurchasesContent = () => (
  <div className="mt-8">
    ...
    <div className="text-center py-24 border rounded-lg bg-gray-50">
      ...
      <button className="mt-6 bg-blue-800 text-white px-6 py-2.5 rounded-md font-semibold hover:bg-blue-900">상품 구매하기</button>
    </div>
  </div>
);
...
const MyProfileContent = () => (
  <div className="text-center py-20 border rounded-lg bg-gray-50">
    ...
    {/* {user && <p>이름: {user.nickname}</p>} – useAuth로 추가 가능 */}
  </div>
);

// MyWishlistContent (가격/저자 에러 고침)
const MyWishlistContent = () => {
  const { wishlist, removeFromWishlist } = useAuth();
  console.log("📋 MyWishlistContent 로드: wishlist 길이 =", wishlist.length);

  const mergedProducts = mergeWishlistWithProducts(wishlist);
        ...
        {mergedProducts.map((item) => (
          <div key={item.id} className="border rounded-lg p-6 flex items-start gap-8 shadow-sm bg-white">
            ...
            <div className="flex-grow">
              ...
              <p className="text-base text-gray-600 mt-2">{item.author || 'Unknown'}</p>
              <p className="text-2xl font-bold mt-4">
                {(item.price || '0').replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,')}원
              </p>
            </div>
            <button
...
// MyInquiriesContent (FAQ + 문의 모달 연동)
...
const MyContentsPage = () => {
  ...
  if (!user) {
    return <div className="text-center py-40 text-gray-600">로그인 후 이용해주세요.</div>;
  }
